library(data.table, warn.conflicts = FALSE) #fread
library(dplyr, warn.conflicts = FALSE) #join, combine, filter, %>%
library(tidyr, warn.conflicts = FALSE) #transponer

options(warn=0)
sink("filtPLP2MGN.log", append=TRUE, split=FALSE)
...
Path_Case <- location
Path_Out <- file.path(location, "OutPLP2M")
