...
filename = r"C:\Users\BH5873\OneDrive - ENGIE\Bureau\plpmanli.dat"
...
with open(filename) as f:
    ...
    for idx, line in enumerate(lines):
        ...
        if '# Bloque         PotMaxAB   PotMaxBA     Operativa\n' in line:
            ...
            df = pd.DataFrame(
                dict_lines_values[current_line], columns=['Etapa', 'AB', 'BA', 'Flag'])
    ...
    df_all_pivot_ab = df_all.pivot(index='Etapa', columns='Line', values=['AB']).fillna(1)
    df_all_pivot_ba = df_all.pivot(index='Etapa', columns='Line', values=['BA']).fillna(1)
    df_all_pivot_flag = df_all.pivot(index='Etapa', columns='Line', values=['Flag']).fillna('T')
    df_all_pivot_ab.to_csv('ab.csv')
    df_all_pivot_ba.to_csv('ba.csv')
    df_all_pivot_flag.to_csv('flag.csv')
