...
def process_marginal_costs(path_case, blo_eta):
    ...
    bar_data = bar_data.rename(columns={"Hidro": "Hyd"}).astype({"Hyd": "float64"})
...
def process_marginal_costs_monthly(bar_data):
    ...
    cmg_m = bar_process(bar_data, m_columns + ["CMgBar"], m_indexes, values="CMgBar")
    dem_m = bar_process(bar_data, m_columns + ["DemBarE"], m_indexes, values="DemBarE")
