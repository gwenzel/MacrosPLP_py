...
def create_plplaja_m(iplp_path: Path, path_inputs: Path):
    ...
    with open(directorio / "plplajam.dat", "w", encoding='latin1') as file:
        ...
        write_float_line(laja_df, 9, 2, 5, decimals=1)
        write_float_line(laja_df, 10, 2, 5, decimals=1)
        write_int_line(laja_df, 11, 2, 5)
        write_int_line(laja_df, 12, 2, 5)
        write_int_line(laja_df, 13, 2, 5)
        write_int_line(laja_df, 14, 2, 5)
        write_int_line(laja_df, 15, 2, 5)
        ...
        file.write(f" 1         {laja_df.iloc[49, 5]:1f}\n")
        file.write(f" 2         {laja_df.iloc[50, 5]:1f}\n")
        file.write(f" 3         {laja_df.iloc[51, 5]:1f}\n")
