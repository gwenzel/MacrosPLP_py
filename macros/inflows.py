...
from utils.utils import (define_arg_parser, get_daily_indexed_df,
                         get_iplp_input_path,
                         check_is_path, process_etapas_blocks)
...
def get_list_of_units(series_inflows):
    return series_inflows.index.get_level_values('CENTRAL').unique().tolist()


def get_list_of_hid(df_all_inflows):
...
def get_df_all_inflows(iplp_path, blo_eta):
    ...
    new_index = ['YEAR', 'MONTH', 'DAY', 'WEEK_NAME', 'ETAPA']
    ...
    df_all_inflows = df_all_inflows.reset_index().drop('WEEK_NAME', axis=1)
    new_index = ['CENTRAL', 'YEAR', 'MONTH', 'DAY', 'ETAPA', 'INDHID']
...
def print_plexos_inflows_all(df_all_inflows, path_pib):
    list_of_hid = get_list_of_hid(df_all_inflows)
    ...
    df_aux = df_aux.drop('ETAPA', axis=1)
    df_aux['PERIOD'] = 1
    df_aux = df_aux[
        ['NAME', 'YEAR', 'MONTH', 'DAY', 'PERIOD'] + list_of_hid]
    # print all data file
...
def shuffle_hidrologies(df_configsim, df_all_inflows):
...
def main():
    ...
    df_all_inflows = shuffle_hidrologies(df_configsim, df_all_inflows)
